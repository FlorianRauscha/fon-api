...
function sonderausgabenXml(s: SonderausgabenSection | undefined): string {
	...
	// 2023 SONDERAUSGABEN sequence: KZ280 → KZ460 (reversed in 2025).
...
function besondereSonderausgabenVerteilungXml(
	...
	// 2023 sequence (re-ordered in 2025; KZ117/KZ118 added in 2025).
	const inner = [
...
/**
 * Build an L1 (ANV) 2023 `<ERKLAERUNGS_UEBERMITTLUNG>` payload from a typed body.
	...
	if (opts.validate !== false) {
		...
		if (!result.success) {
			throw new ValidationError(
				"L1 2023 body failed validation",
