...
function sonderausgabenXml(s: SonderausgabenSection | undefined): string {
	...
	// 2022 SONDERAUSGABEN sequence: KZ280 → KZ460 (reversed in 2025).
...
function abBehindSelfXml(b: RawInnerSection | TypedAbBehindSelf): string {
	...
	const inner = [
		...
		b.aus29bS !== undefined ? `<AUS29B_S>${escapeXml(b.aus29bS)}</AUS29B_S>` : "",
		kzEl("KZ435", b.kz435),
		kzEl("KZ476", b.kz476),
		kzEl("KZ439", b.kz439),
	].join("");
...
function abBehindPartnerXml(b: RawInnerSection | TypedAbBehindPartner): string {
	...
	const inner = [
		...
		b.aus29bP !== undefined ? `<AUS29B_P>${escapeXml(b.aus29bP)}</AUS29B_P>` : "",
		kzEl("KZ436", b.kz436),
		kzEl("KZ417", b.kz417),
		kzEl("KZ418", b.kz418),
	].join("");
...
function fbMonthXml(month: number, m: import("./types.js").FbMonth): string {
	const parts = [
		...
		m.ws !== undefined ? `<FB${month}_WS>${escapeXml(m.ws)}</FB${month}_WS>` : "",
	];
	return parts.join("");
}
...
function besondereSonderausgabenVerteilungXml(
	...
	// 2022 sequence (re-ordered in 2025; KZ117/KZ118 added in 2025).
	const inner = [
...
/**
 * Build an L1 (ANV) 2022 `<ERKLAERUNGS_UEBERMITTLUNG>` payload from a typed body.
	...
	if (opts.validate !== false) {
		...
		if (!result.success) {
			throw new ValidationError(
				"L1 2022 body failed validation",
