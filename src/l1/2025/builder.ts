...
function sonderausgabenXml(s: SonderausgabenSection | undefined): string {
	...
	const inner = [kzEl("KZ460", ts.kz460), kzEl("KZ280", ts.kz280)].join("");
...
function abAllgemeinXml(s: AbAllgemeinSection | undefined): string {
	...
	const inner = [
		t.agbelP !== undefined ? `<AGBEL_P>${escapeXml(t.agbelP)}</AGBEL_P>` : "",
		kzEl("KZ730", t.kz730),
		kzEl("KZ731", t.kz731),
		kzEl("KZ734", t.kz734),
		kzEl("KZ735", t.kz735),
...
/**
 * Build an L1 (ANV) 2025 `<ERKLAERUNGS_UEBERMITTLUNG>` payload from a typed body.
	...
	if (opts.validate !== false) {
		...
		if (!result.success) {
			throw new ValidationError(
				"L1 2025 body failed validation",
