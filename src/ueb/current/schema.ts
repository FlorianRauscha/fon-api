...
/** Signed kz for WEG/ZU. */
const kzvorz = z.number().min(-9_999_999_999.99).max(9_999_999_999.99);
/** Positive kz for UEBERTRAGENDER_BETRAG. */
const kzPos = z.number().min(0.01).max(9_999_999_999.99);
...
const buchungSchema = z
	.object({ aa: aaSchema, zrvon: zeitraumSchema, zrbis: zeitraumSchema, betrag: kzvorz })
	.strict();
...
const erklaerungSchema = z
	.object({
		...
		wegbuchungen: z.array(buchungSchema).max(7).optional(),
		uebertragenderBetrag: kzPos.optional(),
		zubuchungen: z.array(buchungSchema).max(7).optional(),
	})
	.strict();

export const uebBody = z
	.object({
...
export type UebBodyParsed = z.infer<typeof uebBody>;
export { AA_CODES };
