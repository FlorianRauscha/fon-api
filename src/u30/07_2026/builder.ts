...
export interface BuildOptions {
	/** When true (default), validate against the Zod schema before building. */
	validate?: boolean;
}

/**
 * Build a U30 (07_2026) `<ERKLAERUNGS_UEBERMITTLUNG>` XML payload from a typed body.
 * The output is a complete XML document (including the prolog) ready to be passed
 * as the `data` field of a fileupload SOAP call with `art: "U30"`.
 */
	...
	if (opts.validate !== false) {
		...
		if (!result.success) {
			throw new ValidationError(
				"U30 07_2026 body failed validation",
